...
import cn.itcast.lottery.bean.PO.ZCLotteryInfoPO;
import cn.itcast.lottery.bean.VO.LongConnectionInfo;
import cn.itcast.lottery.bean.VO.LotteryInfo;
...
public class NoticeService1 extends Service {
	private static final String TAG = "NoticeService1";
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		impl = BeanFactory.getImpl(ZCLotteryDao.class, NoticeService1.this);
		isOpen = true;
				...
				try {
					...
					if (e instanceof MException) {
						if (((MException) e).getCode() == MException.WRONG_SYNCINFO) {
							...
							// 重新连接
							cn.itcast.lottery.util.Log.info("receive info error");
							startReConnected();
						}
					}
				} finally {
	...
	protected void handleReceiveInfo(String msg) {
		...
		cn.itcast.lottery.util.Log.info("receive info :" + msg);
		// 处理服务器下发的指令
		/**
					...
					case LongConnectionInfo.NOTICE_LOTTERY_WIN:
						...
						if (StringUtils.isNotBlank(lotteryId)) {
							try {
								...
								String message = factory.createMessage(HttpTask.USER_TRANSACTION_DETAILS + "", "13300000000",
										factory.createGetUserTransactionDetails(body), false);
	...
	public void onCreate() {
		Log.i(TAG, "on create");
		startService(new Intent(getApplicationContext(), NoticeService1.class));
		super.onCreate();
	}
	...
	private void sendSystemNotice(String title, String content) {
		...
		boolean vibration = preferences.getBoolean("lottery_vibration", true);
		boolean sound = preferences.getBoolean("lottery_sound", true);
		if (vibration) {
			if (sound) {
		...
		mNotificationManager.notify(R.string.app_name, notification);// 第一个参数为自定义的通知唯一标识
	...
	public void newAlarm(long triggerAtTime, long interval) {
		Intent intent = new Intent(this, NoticeService1.class);
	...
	private boolean openConnection() {
		try {
			String getNowAPN = NetUtil.GetNowAPN(NoticeService1.this);
			...
			public void run() {
				...
				while (socket.isSendSwitch()) {
					...
					try {
						...
						cn.itcast.lottery.util.Log.info("send info:" + item);
						...
						cn.itcast.lottery.util.Log.info("write info error.");
						startReConnected();
					}
				}
			}
	...
	private void getBonuscodeMessage(String lotteryId) {
		ZCLotteryInfoPO result = null;
		Body body = null;
		try {
			body = HttpTask.sendInfo(ConstantValue.URL_LOTTERY, MessageFactory.getInstance().createMessage("14006", "",
					MessageFactory.getInstance().createGetLotterDetailsBody(lotteryId, ""), false), HttpTask.LOTTERY_INFO);
		} catch (Exception e) {
			e.printStackTrace();
			cn.itcast.lottery.util.Log.info(cn.itcast.lottery.util.Log.getTrace(e));
		}

		if (body != null) {
			if (body.lotteryInfoList != null && body.lotteryInfoList.size() > 0) {
				for (LotteryInfo item : body.lotteryInfoList) {
					result = new ZCLotteryInfoPO(item.lotteryname, item.lotteryid, item.issue, item.basecode, item.bonustime,
							item.sjnum, item.jnum, item.qnum, item.salevalue, item.bonusvalue, item.nextbonusvalue, item.topz,
							item.topj, item.onez, item.onej, item.onezjz, item.onezjj, item.twoz, item.twoj, item.twozjz,
							item.twozjj, item.threez, item.threej, item.threezjz, item.threezjj, item.fourz, item.fourj,
							item.fourzjz, item.fourzjj, item.fivez, item.fivej, item.fivezjz, item.fivezjj, item.sixz, item.sixj,
							item.sixzjz, item.sixzjj, item.sevenz, item.sevenj, item.sevenzjz, item.sevenzjj, item.eightz,
							item.eightj, item.eightzjz, item.eightzjj, item.ninez, item.ninej, item.tenz, item.tenj, item.txtj,
							item.txtz, "", "", "", "");
				}
			}
		}
		...
		if (result != null) {
			sendSystemNotice(result.getLotteryname() + "第" + result.getIssue() + "期" + "开奖号码", result.getBasecode());
			// ZCLotteryInfoDao dao = BeanFactory.getImpl(ZCLotteryInfoDao.class, NoticeService1.this);
			// dao.insert(result);
		}
	...
	// private void aa() {
	// NotificationManager mNotificationManager = (NotificationManager) getSystemService(NOTIFICATION_SERVICE);
	// Intent mIntent = new Intent(NoticeService1.this, MainActivity1.class);
	// mIntent.addFlags(Intent.FLAG_ACTIVITY_REORDER_TO_FRONT);
	// PendingIntent mPendingIntent = PendingIntent.getActivity(NoticeService1.this, 0, mIntent, 0);
	//
	// Notification mNotification = new Notification();
	//
	// mNotification.icon = R.drawable.icon;
	//
	// mNotification.setLatestEventInfo(NoticeService1.this, "业务通", "", mPendingIntent);
	// mNotification.flags = Notification.FLAG_ONGOING_EVENT;
	//
	// mNotificationManager.notify(1, mNotification);
	//
